...
def process_image(cap, show=False, store=None, images=False, calc_features=True, save_images=True):
  ...
  if length < seq_length:
    interval = list(range(seq_length))
  ...
  for index in interval:
    ...
    #inp = cv.dnn.blobFromImage(image,1, (224, 224), (104, 117, 123), swapRB=True)
...
def adjust_points(points):
  ...
  if points[BODY_PARTS["Neck"]] is not None:
    ...
    if point_shoulder is not None:
        #print('POINTS', point_shoulder, point_neck)
        ...
        #print('SIZES', vector_size, vector_rate)
        if vector_rate == 0:
          return frame
    ...
    for folder in tasks:
        ...
        for video in data:
            for uttr in data[video]:
                for coords in data[video][uttr]:
                    adjust_points(coords)
            break
        break
...
if __name__ == '__main__':
  ...
  if args.input is None:
    ...
    if task == 'All':
        ...
        tasks = [task]
    for folder in tasks:
      explore_dirs(folder, calc_features=calc_feat)
    join_data()
